...
for j in range(10):
  ...
  hh = torch.matmul(a_norm.view((a_norm.shape[0],1)),b_norm.view((1,9)))
...
def test(epoch):
    ...
    with torch.no_grad():        
        ...
        for batch_idx, (img, label) in enumerate(testloader):
          ...
          #img = torchvision.transforms.functional.affine(img, angle = 120, translate = (0.1, 0.3), scale = 0.5, shear = 0.2)         
