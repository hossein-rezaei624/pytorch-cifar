...
def test(epoch):
    ...
    with torch.no_grad():
        ...
        print("jjjjjjjjjjjjjjjjj",((((((img[0].permute(1,2,0).cpu().numpy())-(img[0].permute(1,2,0).cpu().numpy()).mean())/((img[0].permute(1,2,0).cpu().numpy()).std()))*255.0).astype(np.uint8)).shape))
